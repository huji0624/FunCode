...
- (void) _initViewWithTitle:(NSString *)title andText:(NSString *)text andCancelButton:(BOOL)hasCancelButton forAlertType:(AlertType)type andColor:(UIColor*) color
{
    ...
    CGFloat margin = 5;
    CGFloat width = alertView.bounds.size.width/4*3;
    CGFloat top = CGRectGetMaxY(_titleLabel.frame)+margin;
    CGFloat bottom = CGRectGetMinY(_defaultButton.frame)-margin;
    _textLabel.frame = CGRectMake((alertView.bounds.size.width-width)/2, top, width, bottom-top);
...
- (void) labelSetupWithTitle:(NSString*) title andText:(NSString*) text
{
    ...
    _textLabel = [[UITextView alloc]initWithFrame:CGRectMake(0, 0, 0, 0)];
...
- (void) buttonSetupForType:(AlertType)type withCancelButton:(BOOL) hasCancelButton andColor:(UIColor*) color
{
    ...
    if (hasCancelButton) {
        ...
        _defaultButton.center = CGPointMake(alertView.frame.size.width/2, alertView.frame.size.height-30);
    }
